...
$accepted_formats =["%B %d, %Y", "%B %d, %Y %Z", "%Y%m%d", "%Y%m%d%H%M%S",
                    ...
                    "%Y-%m-%d %H:%M:%S %Z", "%Y/%m/%d %I:%M:%S %p","%m/%d/%Y %I %p", "%d, %B, %Y"]

def parse_date(date, timezone, acc_formats = $accepted_formats)
    acc_formats.each do |date_format|
        begin
            ...
            if date
                case timezone
                    ...
                    when "PDT"
                        ...
                        return "Unrecognized timezone"
                end
            end
        rescue 
        end
    end
    return "Unsupported date format is entered."
end
...
puts parse_date(date_value, type)